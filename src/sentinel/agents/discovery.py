...
SERVICE_SIGNATURES = {
    ...
    "workstation": {
        "ports": [22, 3389, 5900, 5901],
        "services": ["ssh", "rdp", "vnc"]
    },
    "printer": {
        "ports": [9100, 515, 631],
        "services": ["jetdirect", "lpd", "ipp"]
    },
    "camera": {
    ...
    "storage": {
        "ports": [111, 2049, 445, 139, 3260],
        "services": ["nfs", "smb", "cifs", "iscsi"]
    }
}
...
INFRASTRUCTURE_SIGNATURES = {
    "mikrotik": {
        "oui_prefixes": ["00:0C:42", "08:55:31", "2C:C8:1B", "4C:5E:0C", "64:D1:54",
                        "6C:3B:6B", "74:4D:28", "B8:69:F4", "C4:AD:34", "CC:2D:E0",
                        "D4:01:C3", "DC:2C:6E", "E4:8D:8C"],
        "ports": [80, 443, 8291, 8728, 8729],  # Winbox and API ports
        "hostname_patterns": ["mikrotik", "crs", "ccr", "rb", "hex", "hap"],
        "integration_type": "mikrotik",
    },
    "ubiquiti_unifi": {
        "oui_prefixes": ["00:27:22", "04:18:D6", "18:E8:29", "24:5A:4C", "44:D9:E7",
                        "68:72:51", "74:83:C2", "78:45:58", "80:2A:A8", "B4:FB:E4",
                        "DC:9F:DB", "F0:9F:C2", "FC:EC:DA"],
...
class DiscoveryAgent(BaseAgent):
    ...
    async def _direct_network_scan(self, network: str) -> list[Device]:
        ...
        try:
            ...
            # Use TCP connect to common ports to discover hosts
            ...
            for entry in arp_entries:
                ...
                devices.append(device)
                logger.debug(f"Discovered: {ip} ({mac}) - {device.fingerprint.vendor or 'Unknown vendor'}")
    ...
    async def _tcp_probe(self, ip: str) -> bool:
        ...
        Tries to connect to common ports to check if host is alive.
        This also populates the system ARP cache.
        """
        import socket

        # Common ports to try (in order of likelihood to be open)
        ports_to_try = [80, 443, 22, 445, 139, 21, 23, 3389, 8080, 53]
    ...
    async def _scan_ports(
        ...
        if ports is None:
            ...
            ports = [
                22, 23, 80, 443, 445, 139,  # Common services
                3389, 5900, 5901,           # Remote desktop
                8080, 8443, 9090,           # Web alternatives
                3306, 5432, 6379, 27017,    # Databases
                1883, 8883,                 # MQTT
                554, 8554,                  # RTSP
                9100, 515, 631,             # Printing
                111, 2049, 3260,            # Storage
            ]
    ...
    def _classify_device(self, fingerprint: DeviceFingerprint) -> DeviceType:
        ...
        for device_type, sigs in [
            ...
            port_matches = sum(
                1 for p in fingerprint.open_ports
                if p in sigs["ports"]
            )
            if port_matches >= 2:
                return device_type
    ...
    async def _check_infrastructure_device(self, device: Device) -> None:
        ...
        for infra_type, signature in INFRASTRUCTURE_SIGNATURES.items():
            ...
            if score >= 0.4:
                ...
                logger.info(
                    f"Discovered {infra_type} infrastructure at {ip} "
    ...
    async def _propose_segmentation(self, device: Device) -> None:
        ...
        recommended_vlan = VLAN_RECOMMENDATIONS.get(
            device.device_type,
            VLAN_RECOMMENDATIONS[DeviceType.UNKNOWN]
        )
    ...
    async def _update_topology(self) -> None:
        ...
        if switch:
            try:
                ...
                # This would create TopologyNodes and NetworkLinks
            except Exception as e:
        ...
        await self.engine.event_bus.publish(Event(
            ...
            description=f"Topology scan completed with {len(self._inventory.devices)} devices",
            data={"device_count": len(self._inventory.devices)}
        ))
    ...
    async def _capture_rollback_data(self, action: AgentAction) -> Optional[dict]:
        ...
        if action.action_type == "assign_vlan":
            ...
            if device:
                return {
                    "previous_vlan": device.assigned_vlan,
                    "mac": mac
                }
        return None
    ...
    async def _get_relevant_state(self) -> dict:
        ...
        return {
            ...
            "device_types": {
                dtype.value: len(self._inventory.get_by_type(dtype))
    ...
    def get_infrastructure_by_type(self, infra_type: str) -> list[dict]:
        ...
        return [
            info for info in self._discovered_infrastructure.values()
    ...
    async def get_discovery_summary(self) -> dict:
        ...
        return {
            ...
            "online_devices": len([
                d for d in self._inventory.devices.values()
