...
class RouterIntegration(BaseIntegration):
    ...
    # QoS / Traffic Shaping methods (optional - default implementations)
    async def add_traffic_shaper(self, policy: dict) -> str:
        """
        Add a traffic shaping / QoS policy.

        Args:
            policy: QoS policy with:
                - name: Policy name
                - priority_queue: Queue priority (1=highest, 5=lowest)
                - bandwidth_limit_mbps: Max bandwidth (optional)
                - bandwidth_guarantee_mbps: Min guaranteed bandwidth (optional)
                - dscp_marking: DSCP value to apply (optional)
                - destination_port: Match destination port (optional)
                - protocol: Match protocol (optional)

        Returns:
            Policy/rule ID
        """
        logger.warning(f"Traffic shaping not implemented for {self.__class__.__name__}")
        return ""

    async def delete_traffic_shaper(self, policy_id: str) -> bool:
        """
        Delete a traffic shaping policy.

        Args:
            policy_id: Policy ID returned from add_traffic_shaper
    ...
    async def get_traffic_shapers(self) -> list[dict]:
        """
        Get all traffic shaping policies.
