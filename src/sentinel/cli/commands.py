...
app = typer.Typer(name="sentinel", help="AI-Native Security Platform CLI")
console = Console()
...
def start(
    config: str = typer.Option(
        "config/homelab.yaml", "--config", "-c", help="Path to configuration file"
    ),
...
def agents(
    ...
    agent_name: Optional[str] = typer.Argument(None, help="Agent name (for enable/disable)"),
):
    """Manage AI agents."""
    if action == "list":
...
@app.command()
def devices(
    action: str = typer.Argument("list", help="Action: list, scan, classify"),
    filter: Optional[str] = typer.Option(
        None, "--filter", "-f", help="Filter by type: workstation, server, iot, etc."
    ),
):
